...
local TweenInformation = Settings["LightingTweenInformation"]
...
local function BuildSettingsTables()
	...
	for i = 1, #LightingDescendants do
		if LightingDescendants[i]:IsA("ModuleScript") then
			if LightingSettingsTable[LightingDescendants[i].Name] == nil then
				LightingSettingsTable[LightingDescendants[i].Name] = require(LightingDescendants[i])
				Count = Count + 1
			else
				warn("Lighting Setting already exists for ".. LightingDescendants[i].Name.. ".  Make sure settings with the same name do not exist")
			end
		end
	end
	...
	for i = 1, #WeatherDescendants do
		if WeatherDescendants[i]:IsA("ModuleScript") then
			if WeatherSettingsTable[WeatherDescendants[i].Name] == nil then
	...
	if LightingSettingsTable[LightingName] then
		...
		if Settings["ClientSided"] == false or RunService:IsClient() then
			...
			if RunService:IsServer() then
				LightingRemote:FireAllClients("Lighting", LightingName, "Tween")
			end
		end
	else
